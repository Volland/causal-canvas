...
Format is CausalJSON with extension `.causal.json`; product is Causal Canvas; packages are `@causal-canvas/*`. Schema and context are hosted on a registered domain. Code is Apache-2.0, specification CC-BY-4.0.
...
Canvas actions become `jsonc-parser` edits applied as workspace edits, and that translation lives in `@causal-canvas/edits`, not in the extension.
