...
- Reimplementing analysis. The extension surfaces what `@causal-canvas/core` already computes.

## Decisions

### E1 — Canvas edits are surgical text edits in a separate package

**Decision.** Every canvas action is translated into a `jsonc-parser` edit against the document text and applied as a `WorkspaceEdit`. That translation lives in a new `@causal-canvas/edits` package, not in the extension.
...
**Decision.** The preview pane displays exactly what `@causal-canvas/render` emits for the active view, refusing to render when the document has error-severity diagnostics.
...
**The UI layer cannot be tested in this environment** → Push everything testable out of it: edits into `@causal-canvas/edits`, geometry into `@causal-canvas/render`, diagnostics into `@causal-canvas/core`. What remains untested is rendering and pointer handling, verified by build and by manual smoke testing.
